...
import formatDateStringTodMMMMyyyy from './formatDateStringTodMMMMyyyy'

jest.mock('date-fns', () => ({
  format: jest.fn(),
}))

describe('formatDateStringTodMMMMyyyy', () => {
  afterEach(() => {
    jest.clearAllMocks()
  })
  ...
  it('should return "N/A" if params.value is falsy', () => {
    ...
    const result = formatDateStringTodMMMMyyyy(params)
  ...
  it('should return formatted date string using date-fns.format', () => {
    ...
    ;(format as any).mockReturnValue('1 January 2023')

    const result = formatDateStringTodMMMMyyyy(params)

    expect(result).toBe('1 January 2023')
    expect(format).toHaveBeenCalledWith(inputDate, 'd MMMM yyyy')
  })
